...
func NewPushWorker(
	...
	subKey := pubsub.PriorityTopicKey(string(domain.ChannelPush), string(priority))
	return &PushWorker{
		base: newBaseWorker(
	...
	return w.base.subscriber.Subscribe(ctx, w.base.subscription, func(ctx context.Context, msg *pubsub.Message) error {
		...
		if msg.ForcedVendor != "" {
			return w.base.dispatch(ctx, msg, func(ctx context.Context, n *domain.Notification) (domain.DeliveryResult, error) {
				...
				return domain.DeliveryResult{}, fmt.Errorf("forced vendor %q not configured for push channel", msg.ForcedVendor)
			}, msg.ForcedVendor)
		}
		...
		return w.base.dispatch(ctx, msg, func(ctx context.Context, n *domain.Notification) (domain.DeliveryResult, error) {
			...
			if effectiveCfg != w.base.cfg {
				senders = provider.InitializePushSenders(effectiveCfg.Providers.Push)
				routing = effectiveCfg.Providers.PushRouting
			}
			...
			case "only":
				if only == "" {
					return domain.DeliveryResult{}, fmt.Errorf("push routing mode=only requires routing.only (or routing.prefer)")
				}
				for _, s := range senders {
				...
				return domain.DeliveryResult{}, fmt.Errorf("push routing vendor %q not configured", only)
...
func (w *PushWorker) Reload(ctx context.Context, cfg nsconfig.ProviderConfig) {
	...
	w.routing = cfg.PushRouting
	w.mu.Unlock()
	w.base.log.Info("push worker reloaded with new configuration",
		zap.String("routing_mode", cfg.PushRouting.Mode),
		zap.String("routing_prefer", cfg.PushRouting.Prefer),
		zap.String("routing_only", cfg.PushRouting.Only),
	)
}
