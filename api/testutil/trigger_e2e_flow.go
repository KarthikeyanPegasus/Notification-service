...
func main() {
	...
	fmt.Printf("🚀 Triggering notification flow for %s...\n", reqBody.Recipient)
	...
	fmt.Printf("✅ Notification accepted! ID: %s\n", notificationID)
	...
	for {
		select {
		case <-timeout:
			log.Fatalf("❌ Timeout reached waiting for notification to be sent.")
		case <-ticker.C:
			...
			if status == "sent" || status == "delivered" {
				fmt.Printf("\n✨ SUCCESS! Entire flow completed. Notification %s is now %s.\n", notificationID, status)
				return
			}
